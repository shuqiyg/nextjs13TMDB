...
export default async function MovieDetail({params}){
    ...
    const data = await fetch(`https://api.themoviedb.org/3/movie/${movie}?api_key=${process.env.TMDB_API_KEY}`, {next : {revalidate:60}})
    const res = await data.json()
    console.log(res)
    return (
        <div>
            <div>
                ...
                <Image className="my-12 w-full" src={imagePath + res.backdrop_path} width={800} height={800} alt={res.id}/>
            </div>
        </div>
    )
} 